...
import { getDetailCandidateProfileAction } from "../../../../redux/actions/Users/candidateProfileAction/candidateProfileAction";
import {
  Row,
  Col,
  Typography,
...
function CandidateProfileDetailPage() {
  ...
  const { detailCandidateProfile } = useSelector(
    (state) => state.candidateProfileReducer
  );

  useEffect(() => {
    dispatch(getDetailCandidateProfileAction(candidateProfileId));
  }, [dispatch, candidateProfileId]);
  const detail = detailCandidateProfile || {};

  const { user, company, position, status, skillSets } = detail;
  return (
