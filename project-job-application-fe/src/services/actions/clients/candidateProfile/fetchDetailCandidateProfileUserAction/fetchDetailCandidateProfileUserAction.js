...
import { SET_FETCH_DETAIL_CANDIDATEPROFILE_USER } from "@/types";
...
export const fetchDetailCandidateProfileUserAction = (id) => {
  return async (dispatch) => {
    try {
      ...
      const result =
        await clientsMangeCandidateProfile.fetchDetailsCandidateProfile(id);
      if (result.status === STATUS_CODE_OK) {
        dispatch({
          type: SET_FETCH_DETAIL_CANDIDATEPROFILE_USER,
          payload: result.data,
        });
      }
